...
_twisted_prepare_test() {
	...
	if [[ "${EUID}" -eq 0 ]]; then
		...
		sed \
			...
			-i src/twisted/test/test_plugin.py
	fi

	# Remove tests known to fail due to the network sandbox
	rm -R src/twisted/pair/test/test_*.py || die "rm twisted/pair/test/test_*.py FAILED"
	sed \
		-e "s/test_loggingFactoryOpensLogfileAutomatically/_&/" \
		-i src/twisted/test/test_policies.py
	sed \
		-e "s/testLookupProcNetTcp/_&/" \
		-i src/twisted/test/test_ident.py
	...
	sed \
		...
		-i src/twisted/test/test_twistd.py

	# disable due to removing some tests from installation
	sed \
		-e "s/test_exist/_&/" \
		-i src/twisted/python/test/test_dist3.py

	# Display access is not available
	#sed \
	#	-e "s/test_gtkApplicationActivate(self):/_&/" \
	#	-i src/twisted/internet/test/test_gireactor.py
}
