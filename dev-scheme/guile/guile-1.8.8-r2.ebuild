...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		# Move files back.
		if path_exists -o "${ED}"/tmp/guile-config.*; then
			mv "${ED}"/tmp/guile-config.* "${ED}"/usr/bin || die
		fi
	else
		# Preserve ABI-variant of guile-config,
		# then drop all the executables
		mkdir -p "${ED}"/tmp || die
		mv "${ED}"/usr/bin/guile-config "${ED}"/tmp/guile-config.${ABI} || die
		rm -r "${ED}"/usr/bin || die
	fi
}
